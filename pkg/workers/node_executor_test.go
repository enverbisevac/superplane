...
func Test__NodeExecutor_PreventsConcurrentProcessing(t *testing.T) {
	...
	//
	go func() {
		executor1 := NewNodeExecutor(r.Encryptor, r.Registry, "http://localhost", "http://localhost", "", r.AuthService, nil)
		results <- executor1.LockAndProcessNodeExecution(execution.ID)
	}()

	go func() {
		executor2 := NewNodeExecutor(r.Encryptor, r.Registry, "http://localhost", "http://localhost", "", r.AuthService, nil)
...
func Test__NodeExecutor_BlueprintNodeExecution(t *testing.T) {
	...
	//
	executor := NewNodeExecutor(r.Encryptor, r.Registry, "http://localhost", "http://localhost", "", r.AuthService, nil)
